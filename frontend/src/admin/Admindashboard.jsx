...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchbooks = async () => {
      try {
        const res = await axios.post(`${config.apiUrl}/AdminDashboard`, null, {
          headers: {
        ...
        setAvailableBooks(res.data.users || []);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative mt-10 px-4">
        <h2 className="text-xl font-bold text-gray-800 mb-6 text-center">📚 Available Books</h2>
        ...
        {availableBooks.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {availableBooks.map((book) => (
              <div
                key={book.id}
                className="relative bg-white rounded-lg shadow-md p-4 w-full h-40 max-w-xs mx-auto flex flex-col justify-between hover:shadow-xl transition duration-300 cursor-pointer"
              >
                {/* Simulated spine */}
